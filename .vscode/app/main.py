...
def my_applications_page(user_id: int):
    ...
    for app in applications:
        ...
        with st.expander(f"{status_color} {app.application_no} - {material.name if material else '未知'} x{app.quantity} - {status_text}"):
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f"**用途：** {app.purpose}")
                st.markdown(f"**借用时间：** {app.start_time.strftime('%Y-%m-%d %H:%M')} 至 {app.end_time.strftime('%Y-%m-%d %H:%M')}")
            with col2:
...
def pending_approvals_page():
    ...
    for app in pending:
        ...
        with st.container(border=True):
            ...
            with col2:
                st.markdown(f"**用途：** {app.purpose}")
                st.markdown(f"**借用时间：** {app.start_time.strftime('%m-%d %H:%M')} 至 {app.end_time.strftime('%m-%d %H:%M')}")
            with col3:
...
def material_manage_page():
    ...
    for m in materials:
        ...
        with st.container(border=True):
            ...
            with col2:
                new_total = st.number_input("总数量", value=m.total_quantity, key=f"total_{m.id}", label_visibility="collapsed")
            ...
            with col3:
                new_location = st.text_input("位置", value=m.location or "", key=f"loc_{m.id}", label_visibility="collapsed")
